# Module 2: 🗃️ Dense Vector Retrieval
...
The **code** we’ll focus on during this module follows the concepts directly. We will develop a RAG application from scratch in pure Python. Starting first with a primer on understanding embeddings, we will proceed to build a custom **vector store**, implement a **similarity search engine**, and put it all together, Pythonically 🐍. You will be asked, as in module 1, to enhance your Personal Wellness Assistant. As we will see, there are many ways to do this, even with a simple RAG application.
